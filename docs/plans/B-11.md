...
Worker capacity can increase for independent tasks without breaking sequential guarantees or losing work during integration.

### Implementation tasks

- [ ] B-11.1 Reuse B-9 dependency readiness. Run only tasks whose prerequisites are met, each in isolated workspaces and fresh sessions.
- [ ] B-11.2 Honor a shared resource budget across all model phases. Preserve the one-worker default and optionally distinguish model-endpoint capacity where the actual deployment requires it.
- [ ] B-11.3 Serialize integration into the destination. Record each task's original base, candidate, and current destination.
- [ ] B-11.4 Integrate concurrent results in a dedicated clean integration workspace with an explicit merge/cherry-pick policy. Never modify the user's dirty checkout or rewrite published branches.
- [ ] B-11.5 Review affected interactions and re-run required checks against the combined tree. Treat conflict resolution as a bounded agent task or explicit operator action, not an unbounded Git phase.
- [ ] B-11.6 Release dependent tasks only after all prerequisites are present in a verified integrated revision. Preserve both task commits if integration is blocked.

### Acceptance and tests

Two independent branches integrate into one target, and a dependent task sees both results. Test overlapping edits, API changes, integration-test failure, restart under contention, and remote advancement. Use fake model workers in CI; real multi-worker local inference is a later hardware qualification.

### Settings and deployment

A worker limit greater than one is opt-in. The serial integration lock remains required even when multiple model workers are enabled.

### Handoff

Document the verified concurrency limit and distinguish simulated scheduler qualification from real-model performance testing.
