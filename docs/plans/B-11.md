...
Part of the **Kanban reliability and sequential delivery implementation plan**. This file contains the B-11 milestone plus the shared plan context needed to understand and execute it. The full plan, including cross-milestone coordination, lives in [B_IMPLEMENTATION_PLAN.md](./B_IMPLEMENTATION_PLAN.md).

| Field                   | Value                                                                                                                                                   |
| ----------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Document revision       | 4                                                                                                                                                       |
| Prepared                | 2026-09-15 (revision 2: 2026-09-16, revision 4: 2026-09-23)                                                                                             |
| Status                  | B-11 implemented behind opt-in `taskDispatchPolicy` with `workerLimit` > 1 (see "Implementation status"); shared B-0 deployment progress recorded below |
...
| Prerequisites           | B-9, B-10                                                                                                                                               |
| Upstream approach       | Preserve existing compatibility                                                                                                                         |
...
- [x] B-11.1 Reuse dependency readiness from B-9. Run only tasks with satisfied prerequisites, each in an isolated workspace and fresh session.
- [x] B-11.2 Honor a shared resource budget across all model phases. Retain one-worker defaults; optionally distinguish model-endpoint capacity where actual deployment requires it.
- [x] B-11.3 Serialize destination integration. Record each task's original base, candidate, and current destination.
- [x] B-11.4 Integrate concurrent results in a dedicated clean integration workspace with an explicit merge/cherry-pick policy. Never mutate a dirty user's checkout or rewrite a published branch.
- [x] B-11.5 Review affected interactions and rerun required checks against the combined tree. Conflict resolution becomes a bounded agent task or explicit operator action, not an unbounded Git phase.
- [x] B-11.6 Release dependent tasks only after every prerequisite is present in the verified integration revision. Preserve both task commits when integration is blocked.

### Implementation status — 2026-09-23

B-11 is implemented on top of the B-9 opt-in `taskDispatchPolicy` runtime
setting: `workerLimit` > 1 enables parallel dispatch. The default remains one
worker, and with the policy disabled behavior is unchanged.

| Area                                          | Location                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| --------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Shared worker budget (B-11.2)                 | `src/task-dispatch/task-dispatch-service.ts` — `listWorkerSlotSummaries` + `getActiveWorkerTaskIds` count running terminal and cline sessions plus live review/repair sessions; `baseTaskIdForReviewSessionId` maps `<task>::review` / `<task>::verification-repair-N` back to the base task, so a task and its own review/repair sessions hold a single slot. `dispatchReadyTasks` fills up to `workerLimit - activeWorkers` per pass                                                                         |
| Dispatch status (B-11.2)                      | `getTaskDispatchStatus` reports `activeWorkerTaskIds` (every slot holder, deduplicated) alongside the legacy singular `activeWorkerTaskId` (`runtimeTaskDispatchStatusResponseSchema` in `src/core/api-contract.ts`)                                                                                                                                                                                                                                                                                           |
| Isolated-workspace dispatch (B-11.1)          | B-9 behavior unchanged, now exercised under parallel load: each task runs in its own `~/.cline/worktrees/<taskId>/...` worktree with a fresh session (integration suite dispatches two independent tasks in one pass under a budget of 2)                                                                                                                                                                                                                                                                      |
| Recorded base vs current destination (B-11.3) | Handoff `baseSha` (recorded base) is compared against the live destination tip in `integrate()`; `destAdvancedFromRecordedBase` detects the diverged-destination case before any ref move                                                                                                                                                                                                                                                                                                                      |
| Clean integration workspace (B-11.4)          | `integrateDivergedDestination()` in `src/workspace/git-delivery.ts` — disposable detached worktree under `~/.cline/worktrees/kanban-integration/<repo>/<branch>`; `git merge --no-ff` (merge strategy) or `git cherry-pick` (fast_forward strategy); a dirty destination checkout pauses pre-flight; the ref advances via expected-old `update-ref`, or an in-place `--ff-only` merge when the destination is checked out clean (keeping that checkout consistent); the integration worktree is always removed |
| Combined verification (B-11.5)                | `runCombinedVerification` seam on `StartGitDeliveryInput` runs against the integrated tree **before** the destination ref advances; a failure pauses and records `combinedVerificationPassed: false` on the receipt (`RuntimeGitDeliveryReceipt.combinedVerificationPassed: boolean \| null`, `null` = not run); a conflict pauses with the conflict as an explicit operator action — nothing auto-resolved, nothing pushed                                                                                    |
| Dependent unlock (B-11.6)                     | `evaluateDependentsUnlock` unchanged (delivered receipts only); `resolveDispatchBaseSha` verifies the base contains **every** delivered prerequisite's integrated SHA, so a dependent unlocks only once all parallel results are in the verified revision; a blocked integration preserves the task commit (durable task-commit ref + receipt) for retry after manual resolution                                                                                                                               |
| Runtime wiring (B-11.2)                       | `src/trpc/runtime-api.ts` — `listReviewSessionSummaries` resolves the workspace-scoped `ClineReviewSessionService.listSessionSummaries()` (delegates to the task-session service), feeding slot accounting and status; `src/cline-sdk/cline-review-session-service.ts` gains the `listSessionSummaries()` accessor                                                                                                                                                                                             |

Evidence: `test/workspace/git-delivery.test.ts` (B-11 describe: cherry-pick and
no-ff-merge integration of a diverged destination, conflict pause with the
destination untouched and the integration worktree cleaned, dirty-destination
pre-flight pause, combined verification pass/fail recorded on the receipt; the
B-8 diverged-base test now asserts in-place fast-forward of a checked-out clean
destination), `test/task-dispatch/task-dispatch-service.test.ts` (B-11 shared
budget: review sessions count toward `workerLimit`, distinct tasks hold
distinct slots, a task plus its own review/repair sessions holds one slot,
`activeWorkerTaskIds` in the status response; `baseTaskIdForReviewSessionId`
mapping), `test/cline-sdk/cline-review-session-service.test.ts` (summary
delegation), and `test/integration/task-dispatch.integration.test.ts` (B-11
describe: two independent branches dispatched in parallel under a shared budget
of 2, both cherry-picked onto main, then a dependent task dispatched at a base
containing both integrated results — real git repos, `HOME` redirected).

Known gaps (tracked for B-12):

- The budget is a single shared slot pool; distinguishing model-endpoint
	capacity (the optional clause of B-11.2) is not implemented.
- Qualification uses fake model workers in CI; real multi-worker local
	inference is a later hardware qualification.
- Restart under contention (restart while parallel deliveries are in flight)
	is covered by the per-workspace dispatch lock and reconciliation, not by an
	explicit concurrent-restart test.
