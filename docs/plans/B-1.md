...
Produce concrete failing tests and evidence for every proposed repair, including the commit and cleanup failures.

### Implementation tasks

- [ ] B-1.1 Trace implementation, review entry, manual commit, auto-commit, session restart, and Done/cleanup across CLI, backend, and UI. Document where provider calls actually occur.
- [ ] B-1.2 Capture sanitized reproduction evidence: effective provider/model metadata, configured/served context capacity, reported input/output usage, requested output headroom, phase, request identifier, SDK version, and exception code/shape. Do not capture full prompts or secrets by default.
- [ ] B-1.3 Inspect the effective Git prompt and project rules. Distinguish full review from conflict resolution or repeated file reads. Record whether failures occur before helper/tool invocation.
- [ ] B-1.4 Build a fake OpenAI-compatible provider with a configurable small context limit and fixture error responses matching observed failures. Use it for fast automated tests without loading the real local model.
- [ ] B-1.5 Add a disposable Git fixture with a main checkout, detached task worktree, and local bare origin. Reproduce manual commits during auto-completion, failed pushes, service interruption, and clean-but-unintegrated work.
- [ ] B-1.6 Read existing upstream issues and all comments before drafting related issues. Separate confirmed bugs from feature proposals. Do not post private work in public logs.

### Acceptance and tests

At least one context failure and one incorrect-completion/cleanup scenario reproduce deterministically. Tests identify the failing boundary without relying on the real SDK subprocess starting in a unit suite.

### Settings and deployment

Diagnostics are off or minimal by default, opt-in for detailed metadata, and size-limited. This milestone changes no production behavior.

### Handoff

The short evidence report lists observed facts, hypotheses, exact fixtures, failing assertions, and recommended ownership: Kanban, SDK, provider configuration, or model server.
