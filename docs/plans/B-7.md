...
Part of the **Kanban reliability and sequential delivery implementation plan**. This file contains the B-7 milestone plus the shared plan context needed to understand and execute it. The full plan, including cross-milestone coordination, lives in [B_IMPLEMENTATION_PLAN.md](./B_IMPLEMENTATION_PLAN.md).
...
| Prerequisites | B-4, B-6 |
| Upstream approach | Shared verification service |
...
Configured checks, not agent narrative, determine whether the reviewed content is eligible for delivery.
...
- [ ] B-7.2 Freeze agent writes during verification. Record candidate-tree identity before and after checks. Preserve full logs locally, with bounded excerpts for the model/UI.
- [ ] B-7.3 Capture exit status, timeout/cancellation, start/end time, tested revision/tree, and artifact path. A missing required check, interrupted test, and empty output are not success without a configured exit semantic.
- [ ] B-7.4 Treat test-induced source mutation as invalidating prior review/verification where appropriate. Do not accidentally commit unreviewed formatter changes or test-generated sources.
- [ ] B-7.5 Feed failed-check evidence into a new bounded repair session with existing rules, then re-run required checks. Share one explicit attempt budget so review/verification cannot loop indefinitely.
- [ ] B-7.6 Bind approved evidence to the candidate that will be committed. Re-run checks after integration whenever the resulting tree differs from the verified tree.

### Acceptance and tests

Cover successful checks, failing tests, timeouts, non-zero exit with plausible success text, huge output, missing executable, post-test edits, generated files, and repair exhaustion. Git publication must not occur on failure.

### Settings and deployment

Project check configuration is trusted executable configuration, not a value an untrusted task document can rewrite to exempt itself. A project without required checks needs an explicit policy decision; the target workflow must not silently skip checks.

### Handoff

The verification receipt contains exact tree identity, checks and results, artifact references, and validity status.
