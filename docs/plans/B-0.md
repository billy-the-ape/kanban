...
Develop on the MacBook, accept cloud-agent changes through fork PRs, and deploy precisely reviewed fork revisions to the server under the existing agent account. Maintain easy updates and rollback.

### Implementation tasks

- [ ] B-0.1 Record the fork/default branch, upstream commit, Node/npm versions, lockfile hash, and installed SDK versions. Verify clean local state and remote mappings. Keep the fork's main aligned with upstream; use a separate repair branch and, only when needed, an explicitly named integration/deployment branch.
- [ ] B-0.2 Verify installation with locked dependencies for both backend and web UI. Run the upstream build and checks. Record pre-existing failures rather than opportunistically repairing unrelated code.
- [ ] B-0.3 Define the Linux deployment layout: source checkout owned by the agent account, a versioned build/release directory, and a stable active-release path. Build native dependencies on Linux; do not copy Mac node_modules. Preserve the existing working directory and model configuration unless intentionally migrating.
- [ ] B-0.4 Prepare a reproducible deployment procedure that takes an explicit commit, verifies clean checkout state, fetches from the fork, builds before cutover, records a manifest, and switches the service only after readiness checks pass.
- [ ] B-0.5 Verify the live system service, effective user, proxy routes, configuration locations, runtime state, and permissions. Drain active work before cutover. Preserve provider configuration and credentials. Permission to discard stale board tasks is not permission to delete source worktrees indiscriminately.
- [ ] B-0.6 Deploy the unmodified baseline first. Smoke-test browser access, a throwaway task, local inference, restart persistence, and branch-write capability in a throwaway repository.
- [ ] B-0.7 Test rollback to the previous release. Keep backups outside public repositories. Update the private homelab documentation with verified paths, commands, revisions, and the rollback procedure.

### Acceptance and tests

A deployment manifest identifies the source SHA and dependency versions. The new service runs the intended build, existing ingress works, a local-model task succeeds, and rollback is demonstrated. A successful Mac build does not imply deployment.

### Settings and deployment

This plan does not require new application environment variables. Record actual build or service requirements discovered during implementation. Preserve installed authentication boundaries. The current environment blocks automated SSH; server commands require an operator unless an authorized supported execution route becomes available.

### Handoff

Record build/test evidence, baseline SHA, Linux release identity, and a documentation PR. Keep operational host details in the private documentation repository rather than the public fork.
