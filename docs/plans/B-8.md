...
Application code commits the verified task, integrates it, pushes it, and verifies delivery even if the model is unavailable.

### Implementation tasks

- [ ] B-8.1 Implement a Git service using existing subprocess/environment conventions and argument arrays. Verify repository/worktree identity, target ref, remote, permissions, and operation state. Never interpolate task text into a shell.
- [ ] B-8.2 Stage only the intended task changes from the recorded change manifest, including reviewed new files. Exclude secrets, generated logs, and unrelated user changes. Use a deterministic task-title message fallback; model-generated messages are optional.
- [ ] B-8.3 Create the commit anchored by a durable ref. Respect Git hooks and signing configuration. A failure remains actionable, not a reason to bypass policy.
- [ ] B-8.4 For the initial sequential policy, require the destination to match the recorded base and fast-forward to the task commit. If checked out, reject dirty state and use the appropriate worktree-aware operation; if not checked out, use an expected-old-SHA ref update.
- [ ] B-8.5 When the destination has advanced or diverged, pause with clear evidence. Do not silently stash user changes, cherry-pick, resolve conflicts, delete lock files, or force-push. Later parallel integration belongs to B-11.
- [ ] B-8.6 Push the explicit ref using existing credentials and honor protected-branch configuration. The target deployment uses a feature branch. Serialize destination integration/publication.
- [ ] B-8.7 Verify the remote equals or contains the delivered commit. If it advanced, fetch the relevant ref to verify ancestry. Remote movement that excludes the candidate blocks completion.
- [ ] B-8.8 Persist the delivery receipt before unlocking dependents. On an ambiguous push timeout after server acceptance, reconcile before retrying; reuse the existing commit rather than creating another.
- [ ] B-8.9 Keep optional PR creation separate from commit/push. If supported here, use the API/CLI with structured metadata, dedupe by head/base, and preserve the pushed commit if PR creation fails. The primary workflow is multiple task commits on one feature branch, followed by an overall PR later.

### Acceptance and tests

Run with a model stub that refuses all requests during delivery. Test a detached worktree, checked-out target, no-op task with explicit evidence, user-created commit, dirty destination, signing/hook failure, push rejection, ambiguous push timeout, remote advancement, process crash, and retry deduplication.

### Settings and deployment

Proposed policy fields: destination branch, remote, push required, protected target, integration strategy, and optional PR requirement. The target sequential workflow requires push. Preserve the legacy custom Git prompt mode as an explicit alternative during migration. Never run both paths in one attempt.

### Handoff

The receipt records the task commit, integrated commit, destination ref, tested tree, remote observation, operation ID, and completion timestamp.
