...
Part of the **Kanban reliability and sequential delivery implementation plan**. This file contains the B-8 milestone plus the shared plan context needed to understand and execute it. The full plan, including cross-milestone coordination, lives in [B_IMPLEMENTATION_PLAN.md](./B_IMPLEMENTATION_PLAN.md).

| Field | Value |
| --- | --- |
| Document revision | 3 |
| Prepared | 2026-09-15 (revision 2: 2026-09-16) |
| Status | implemented 2026-09-22 (B-8.1…B-8.9 in this revision's implementation record); shared B-0 deployment progress recorded below |
...
| Prerequisites | B-5, B-7 |
| Upstream approach | Discuss replacement/opt-in behavior |
...
- [x] B-8.1 Implement a Git service using existing subprocess/environment conventions and argument arrays. Validate repository/worktree identity, target ref, remote, permissions, and operation state. Do not interpolate task text into a shell.
- [x] B-8.2 Stage only intended task changes according to a recorded change manifest, including reviewed new files. Exclude secrets, generated logs, and unrelated user changes. Use a deterministic task-title message fallback; model-generated messages are optional.
- [x] B-8.3 Create a commit anchored by a durable ref. Respect Git hooks and signing settings; failures remain actionable rather than bypassing policy.
- [x] B-8.4 For the initial sequential policy, require the destination to match the recorded base and fast-forward to the task commit. If destination is checked out, refuse dirty state and use the appropriate worktree-aware operation. If not checked out, use an expected-old-SHA ref update.
- [x] B-8.5 If the destination advanced or diverged, pause with clear evidence. Do not silently stash user changes, cherry-pick, resolve conflicts, delete lock files, or force-push. Later parallel integration belongs to B-11.
- [x] B-8.6 Push an explicit refspec using existing credentials, reusing the Git invocation conventions in src/workspace/git-sync.ts rather than its bare ["push"] argument list, which relies on ambient push configuration and cannot state what was delivered. Honor protected-branch settings; the target deployment uses a feature branch. Serialize integration/publication for the destination.
- [x] B-8.7 Verify the remote equals or contains the delivered commit. Fetch the relevant ref for ancestry validation when it advanced. Remote movement that excludes the candidate blocks completion.
- [x] B-8.8 Persist the delivery receipt before unlocking dependents. If push times out after server acceptance, reconcile before retry. Reuse existing commits; do not create a new commit merely because the client lost its response.
- [x] B-8.9 Keep optional PR creation separate from commit/push. If supported here, use API/CLI with structured metadata, deduplicate by head/base, and preserve the pushed commit on PR creation failure. The primary workflow is many task commits on one feature branch, followed by a later overall PR.
...
- `src/workspace/git-delivery.ts` — `GitDeliveryService` (shared singleton via `getGitDeliveryService()`) runs validate → stage → commit → integrate → push → verify → optional PR against the existing `runGit`/`createGitProcessEnv` conventions (argument arrays, sanitized env, bounded output). The git runner and gh runner are injectable for tests.
- Durable receipt at `<task worktrees home>/<taskId>/delivery/receipt.json` (mirrors the review/verification artifact dirs), persisted after every stage; `readTaskDeliveryReceipt`/`getTaskDeliveryInfo` expose it. Durable task commit ref: `refs/kanban/tasks/<taskId>/commit`.
- Integration: when the recorded base (review-handoff `startingCommit`) exists, the destination must match it; a prior successful delivery advances the expected base to its integrated sha; when no base was recorded, the fast-forward guarantee degrades to `merge-base --is-ancestor dest taskCommit`. Checked-out destinations use the worktree-aware `merge --ff-only`/`merge --no-ff` after a clean-tree check; unchecked-out destinations use expected-old-SHA `update-ref` (merge strategy builds the merge commit with `commit-tree` plumbing). Divergence, dirty checkouts, and concurrent ref movement pause with evidence — never force/reset/stash.
- Push is always the explicit refspec `push <remote> refs/heads/<dest>:refs/heads/<dest>` (B-8.6); a failed push is reconciled with `fetch` + `merge-base --is-ancestor <integrated> <remoteSha>` before failing (B-8.8), so an ambiguous timeout never produces a second commit. B-8.7 verification re-fetches and blocks completion when the remote excludes the delivered commit.
- Policy (`gitDeliveryPolicy`) is global, stored in `~/.cline/kanban/config.json` via the existing `runtime-config.ts` nested-policy pattern (`normalizeGitDeliveryPolicy` degrades invalid ref names to defaults; save-time validation throws). Off (absent) by default, so model-driven git behavior is unchanged until enabled.
- API: `runtime.startTaskDelivery` (mutation) + `runtime.getTaskDeliveryInfo` (query) in `app-router.ts`/`runtime-api.ts`; request/response schemas in `api-contract.ts`. The web-ui `use-git-actions` hook routes both commit and PR actions through the deterministic path when the policy is enabled; review/verification gate outcomes are recorded as receipt evidence but never block deterministic delivery.
- Tests: `test/workspace/git-delivery.test.ts` (real temp repos + bare remote: delivered ff, model message, no-op, protected branch, dirty checked-out destination, divergence pause, ambiguous-push reconciliation, push rejection, merge strategy, secret/log exclusions, retry dedup) and `test/runtime/config/runtime-config-git-delivery.test.ts` (normalize/save/merge/clear/global-update).
- Review follow-up (2026-09-23):
  - The durable commit ref moved to `refs/kanban/delivery/<id>` (the B-5
    preservation ref `refs/kanban/tasks/<id>` made the old
    `refs/kanban/tasks/<id>/commit` impossible to create).
  - A clean worktree is no longer a no-op when HEAD holds commits not on the
    destination (agent/manual commits, or a commit left by a crashed
    attempt): those commits are delivered as-is. Interrupted attempts resume
    publication from the receipt; receipts are `in_progress` mid-pipeline.
  - The change manifest parses `git status --porcelain=v2 -z` by field
    position (modified, renamed, deleted, conflicted, space-named paths), and
    staging is `git add -A` then `git restore --staged` for excluded paths, so
    an excluded file an agent staged earlier never reaches the commit.
  - Merge strategy: a destination that already contains the task commit is
    never merged again (fixes the retry-after-push-failure pause and the
    duplicate merge commit on no-change retries).
  - Gates: when review or verification is `required`, delivery pauses unless
    the stored ready verdict / passing receipt is bound to the exact
    candidate tree (B-6.7/B-7.6).
  - Remote observation fetches into `refs/kanban/delivery-remotes/...`
    instead of racing on FETCH_HEAD; a missing local destination is created
    from the remote branch when it exists.
  - PRs: `gh` runs inside the repository, deduplicates with
    `gh pr list --head/--base`, and targets `pullRequestBaseBranch` (null =
    the forge's default branch).
  - B-8.8: `getTaskDeliveryInfo` reports `dependentsUnlock`; the web UI and
    `task complete` start linked tasks only with a completed receipt. This is
    the delivery-side half of B-8.8; the attempt model it should sit on is
    B-4 (still open).
  - CLI: `kanban task deliver` and `kanban task delivery`.
- Known gaps for later milestones: process-crash recovery is exercised indirectly (durable ref + receipt stage resume + retry dedup); the "process crash" and "signing/hook failure" acceptance scenarios are covered by design (hooks/signing are never bypassed; a commit failure persists an actionable `failed` receipt) but not by a dedicated test; PR creation (`gh`) is implemented but untested against a live GitHub.
