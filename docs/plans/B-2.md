...
- [x] B-2.1 Trace model metadata from settings through src/cline-sdk/cline-provider-service.ts, SDK boundaries, and the request pipeline. Verify the LiteLLM-compatible and custom OpenAI-compatible routes used by local inference. (card [B-2-0](./B-2-0.md))
- [x] B-2.2 Define context-limit precedence: explicit validated deployment/model override, then reliable provider metadata, then a documented conservative fallback. Use the most restrictive known served limit. Display the source of the selected limit. The middle tier does not exist yet and depends on B-2.8; until it does, precedence collapses to override then fallback. (card [B-2-2](./B-2-2.md); the metadata tier shipped in [B-2-1](./B-2-1.md))
- [x] B-2.3 Reuse SDK compaction configuration. Budget system text, tool schemas, current messages, incoming file/tool content, expected output, and a safety margin. Do not count the same reserve twice if the SDK already subtracts it. (cards [B-2-3](./B-2-3.md), [B-2-4](./B-2-4.md))
- [x] B-2.4 Trigger before the assembled request would exceed its input budget. Start evaluation around 80% utilization as a proposed tuning value, not a hardcoded universal answer; use measured behavior to choose defaults. (card [B-2-5](./B-2-5.md); the 80% point is the user-tunable margin default, exercised by the B-2-10 0.79/0.81 matrix)
- [x] B-2.5 Bound large file reads, command output, and diff results using pagination or excerpts with local artifact references. A single large result must not bypass the budget merely because previous usage was low. (cards [B-2-6](./B-2-6.md), [B-2-7](./B-2-7.md))
- [x] B-2.6 Apply the same policy to implementation, review, repair, and any optional summarization calls. Serialize local-model usage under the worker limit. (card [B-2-8](./B-2-8.md))
- [x] B-2.7 Expose additive configuration through existing settings/schema paths. If the defect is in SDK estimation or provider request assembly, fix it there and consume a released/pinned dependency rather than patching node_modules. (card [B-2-9](./B-2-9.md))
- [x] B-2.8 Carry context capacity through the runtime contract. runtimeClineProviderModelSchema in src/core/api-contract.ts has no context-window field, so provider metadata cannot currently inform any budget. Add an optional capacity field, populate it from the SDK model list and the LiteLLM /model/info route where each supplies one, and treat absence as unknown rather than unlimited. Reconcile the existing web UI test fixture that already passes contextWindow: null. (card [B-2-1](./B-2-1.md))

### Subtask execution cards

B-2 is executed as granular cards in `docs/plans/B-2-0.md` … `B-2-10.md`, one per execution unit for fresh-context agents, in this order. Each card references back to this milestone and lists its prerequisites; agents must re-read current code before exact edits and record drift from the source baseline.

| Card | B-2 subtask | Scope summary | Depends on |
| --- | --- | --- | --- |
| [B-2-0](./B-2-0.md) | B-2.1 | Metadata trace doc + session-start context diagnostics | B-1 |
| [B-2-1](./B-2-1.md) | B-2.8 | `contextWindow` in the provider-model contract (absent = unknown) | B-2-0 |
| [B-2-2](./B-2-2.md) | B-2.2 | Effective-limit precedence resolver + source display | B-2-1 |
| [B-2-3](./B-2-3.md) | B-2.3 (1/2) | Pure token-budget estimator module | B-2-2 |
| [B-2-4](./B-2-4.md) | B-2.3 (2/2) | Wire `CoreCompactionConfig` into every session start | B-2-2, B-2-3 |
| [B-2-5](./B-2-5.md) | B-2.4 | Proactive trigger on the assembled request; measured defaults | B-2-3, B-2-4 |
| [B-2-6](./B-2-6.md) | B-2.5 (1/2) | Bound file-read results + local artifact references | B-2-4 |
| [B-2-7](./B-2-7.md) | B-2.5 (2/2) | Bound command output + diff results (same mechanism) | B-2-6 |
| [B-2-8](./B-2-8.md) | B-2.6 | Policy across all session phases; single-worker serialization | B-2-4, B-2-5 |
| [B-2-9](./B-2-9.md) | B-2.7 | Additive settings (override/strategy/threshold/reserve/margin) through existing settings paths | B-2-2, B-2-4, B-2-5 |
| [B-2-10](./B-2-10.md) | closing | Acceptance matrix, measurements, B-2.md handoff + patch ledger | B-2-0 … B-2-9 |

Note: the B-2.8 card (B-2-1) executes before B-2.2 (B-2-2) because the metadata precedence tier depends on the contract field; this matches the dependency noted in the B-2.2 task above.
...
Executed by `test/runtime/cline-sdk/context-budget-acceptance.test.ts` (card [B-2-10](./B-2-10.md)): all six scenarios green against the fake provider with zero context-overflow errors; every estimate is labeled in the suite and in the session logs.
...
B-2 is complete as of fork SHA `19e5caa` (card B-2-10, 2026-09-19).

- **Tested effective budget:** 8,000 tokens under an explicit `override` source in every e2e acceptance scenario (the S2 smaller-limit path; the session-start log records `contextLimitSource: "override"`), plus the documented 200,000-token fallback and the override/metadata most-restrictive precedence rules at the provider-service level. Calibrated at 8k (estimates): system prompt 16 + tool schemas 1,298 + safety margin 4,096 → calibrated window 6,686; reserve 4,596 (500 output + 4,096 margin, counted once); trigger 2,090.
- **Before/after request sizes** (estimates, 12-round near-limit fixture): the un-compacted (pre-B-2) history reaches 10,992 message tokens by round 12 and would have overflowed the 8k window around round 7; with B-2 the max assembled request is 3,252 prompt tokens (3,752 including the 500-token output reserve — 46.9% utilization) with zero provider overflow rejections. The 806,727-char tool result is bounded to 4,000 chars at ingestion (follow-up request 2,328 prompt tokens vs ≈201,682 unbounded; full output preserved as a local artifact).
- **SDK responsibility:** @clinebot/core 0.0.38 owns the compaction trigger/strategy plumbing and the `compact` callback surface; its three defects (thresholdRatio ignored when reserveTokens is set, trigger counts messages only, prepareTurn never invoked in local mode — [B-2-5](./B-2-5.md)) leave the SDK trigger inert in local mode. Kanban owns the effective-limit resolver, the assembled-request calibration, the beforeModel compaction hook (the local-mode guard), the afterTool result bounding, and the settings surface.
- **Dependency release needed for deployment:** none. B-2 works on the pinned @clinebot/core 0.0.38 through Kanban-side hooks; no node_modules patches. When the upstream prepareTurn fix ships, the SDK trigger and the hook share the same calibrated numbers, so the hook can be retired (patch ledger below).

### Patch ledger

| B ID | Fork PR | Upstream issue/PR | Dependency revision | Deployment SHA | Retire-able |
| --- | --- | --- | --- | --- | --- |
| B-2 | pending (fork `billy-the-ape/kanban` @ `19e5caa`) | pending — file against @clinebot/core when the fork PR lands: prepareTurn never invoked in local mode; thresholdRatio ignored when reserveTokens is set; trigger counts messages only (per [B-2-5](./B-2-5.md)) | @clinebot/core 0.0.38 (pinned in lockfiles; no node_modules patches) | pending deployment of the fork SHA containing B-2 | Yes — once the SDK releases the prepareTurn fix, retire the beforeModel compaction hook (the calibrated SDK trigger takes over); keep the afterTool bounding (ingestion-time, complementary to the SDK's request-assembly cap) |
