...
Requests fit the actual served model capacity, including implementation and review sessions, without depending on a larger model window.

### Implementation tasks

- [ ] B-2.1 Trace model metadata from configuration through src/cline-sdk/cline-provider-service.ts, the SDK boundary, and request assembly. Validate the LiteLLM-compatible and custom OpenAI-compatible routes used by local inference.
- [ ] B-2.2 Define context-limit precedence: explicit verified deployment/model override, then trustworthy provider metadata, then a documented conservative fallback. Use the most restrictive known served limit and display its source.
- [ ] B-2.3 Reuse SDK compaction settings. Budget system text, tool schemas, current messages, incoming file/tool content, expected output, and a safety margin. Do not double-count the same reserve when the SDK already subtracts it.
- [ ] B-2.4 Trigger before the assembled request exceeds the input budget. Use about 80% utilization as the proposed evaluation starting point, not a hard-coded universal answer; choose defaults from measured behavior.
- [ ] B-2.5 Limit large file reads, command output, and diff results with paging or excerpts that reference local artifacts. A single large result must not bypass the budget because earlier usage was low.
...
- [ ] B-2.7 Expose additive settings through existing configuration/schema paths. If the defect belongs in SDK estimation or provider request assembly, fix it there and consume the released/pinned dependency; do not patch node_modules.

### Acceptance and tests

Tests cover missing or incorrect provider metadata, explicit smaller server limits, output reservation, tool-schema overhead, large next tool results, and near-limit history. Verify the actual request against a fake provider's own tokenizer/limit accounting where practical. Label estimates as estimates.
...
Record the tested effective budget, before/after request sizes, SDK ownership, and the dependency release needed for deployment.
