...
| Status | B-10.1 through B-10.7 implemented (2026-09-23); shared B-0 deployment progress recorded below |
...
- [x] B-10.1 Display meaningful phases: Implementing, Reviewing, Checking, Committing, Integrating, Pushing, Verifying remote, Done, and Needs attention. Done here means the completed state introduced in B-5.8 and must remain visually and behaviorally distinct from discarding a task.
- [x] B-10.2 Show branch, commit, workspace location, last successful phase, blocked reason, and preserved-work status in task details. Offer open/copy actions.
- [x] B-10.3 Provide backend-backed Retry current phase, Resume repair, Cancel, and Recover workspace actions. Disable invalid transitions and deduplicate requests.
- [x] B-10.4 Show context usage as measured or estimated, effective capacity, last compaction event, and omitted-history notice. Do not imply a summary is lossless.
- [x] B-10.5 Add CLI/JSON equivalents for status, receipts, queue control, and recovery. Keep scripting output stable and human errors actionable.
- [x] B-10.6 Add explicit settings for reliable completion, required review/checks, push requirement, and worker capacity. Document how legacy custom prompts behave.
- [x] B-10.7 Export redacted diagnostic bundles only through an explicit user action. Exclude API keys, prompts, private code, and raw environment by default.

### Implementation notes (2026-09-23)

- B-10.1: The `TaskPhaseBadge` renders the full phase set in board cards and the detail view; `idle` renders nothing so untouched tasks stay quiet. `done` is the B-5.8 completed state, rendered green and never conflated with discarding (discarding is not a phase).
- B-10.6: The runtime settings dialog exposes one "Require reliable completion" master switch. The save request carries `reliableCompletion: boolean`, which the runtime expands into the review, verification, git-delivery, and dispatch policies in the same save; explicit per-policy fields in the same request still win per gate. The individual gate states and worker capacity are derived read-only status (the `reliableCompletion` block in the config response), so the toggle and the stored policies can never disagree.
- Custom prompt behavior (B-10.6): Enabling reliable completion does not touch the user's commit/open-PR prompt templates or any per-task custom prompts. They keep applying exactly as before; the pipeline gates only add required review, checks, push, and dispatch on top of the existing agent run.
