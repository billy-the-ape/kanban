...
Part of the **Kanban reliability and sequential delivery implementation plan**. This file contains the B-10 milestone plus the shared plan context needed to understand and execute it. The full plan, including cross-milestone coordination, lives in [B_IMPLEMENTATION_PLAN.md](./B_IMPLEMENTATION_PLAN.md).
...
| Prerequisites | B-2 through B-9 |
| Upstream approach | Thin UI/CLI integration |
...
Users can understand, resume, and recover work without searching hidden directories or raw internal logs.

### Implementation tasks

- [ ] B-10.1 Display meaningful phases: implementing, reviewing, checking, committing, integrating, pushing, remote verifying, done, and needs attention.
- [ ] B-10.2 Show branch, commit, workspace location, last successful phase, blocked reason, and preserved-work status in the task detail. Provide open/copy actions.
- [ ] B-10.3 Provide backend-backed actions: retry current phase, resume repair, cancel, and recover workspace. Disable invalid transitions and deduplicate requests.
- [ ] B-10.4 Display context usage as measured or estimated, effective capacity, last compaction event, and omitted-history notice. Do not imply summaries are lossless.
- [ ] B-10.5 Add CLI/JSON equivalents for status, receipts, queue control, and recovery. Keep scriptable output stable and human errors actionable.
- [ ] B-10.6 Add explicit settings for reliable completion, required review/checks, push requirement, and worker capacity. Document legacy custom-prompt behavior.
...
UI and CLI show the same durable state after reload/restart. A failed push can be resumed without rerunning implementation. Tests cover duplicate clicks, stale state, inaccessible workspaces, and redaction.
...
Provide screenshot/UI test evidence, CLI examples, configuration migration notes, and a troubleshooting guide.
