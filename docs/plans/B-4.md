...
Part of the **Kanban reliability and sequential delivery implementation plan**. This file contains the B-4 milestone plus the shared plan context needed to understand and execute it. The full plan, including cross-milestone coordination, lives in [B_IMPLEMENTATION_PLAN.md](./B_IMPLEMENTATION_PLAN.md).
...
| Prerequisites | B-1 |
| Upstream approach | Reliability foundation |
...
- [x] B-4.1 Define typed completion phases: implementation, review, verification, committing, integrating, integrated verification when needed, pushing, remote verification, complete. Represent blocked/failed/canceled status separately with a resumable phase.
- [x] B-4.2 Persist attempt ID, task ID, repo identity, starting SHA, candidate tree, target ref, session IDs, policy version, review/check evidence, task commit, integrated commit, remote receipt, timestamps, and failure reason. Store logs by reference.
- [x] B-4.3 Use existing durable state conventions and locks. Add schema versioning and migration; old Done cards without receipts must not automatically unlock new reliable-mode tasks.
- [x] B-4.4 Introduce idempotent backend phase commands shared by UI and CLI. Double-clicks, duplicate browser tabs, hooks, and restart events must resolve to one attempt owner.
- [x] B-4.5 Persist intent before Git side effects and reconcile actual state after restart. Handle the crash between a successful operation and its completion record.
- [x] B-4.6 Define cancellation per phase. Do not leave locks permanent; do not assume killing a Git process undoes a ref update.
- [x] B-4.7 Define what happens to an in-flight attempt when reliable mode is disabled or the code is rolled back mid-attempt. An attempt parked at committing, integrating, or pushing must either finish under the mode it started in or stop at a phase an operator can resume by hand; it must never silently fall back to the legacy prompt path partway through. Record the mode and policy version on the attempt so this is decidable after restart.
- [x] B-4.8 Reconstruct existing task launch configuration after process restart instead of relying solely on lastStartRequestByTaskId. Restore model/provider, workspace, rules, tools and approval behavior from supported durable metadata; resolve credentials through existing configuration without copying secrets into task records. Bind the persisted transcript, avoid duplicate sessions/tools, and pass B-1.7 before declaring restart recovery complete. (Delivered: `src/cline-sdk/cline-task-launch-config.ts` persists the credential-free launch config (mode, system prompt, task title, reasoning effort; schema v1) into the SDK session record metadata via read-merge-write on every start, and `cline-session-runtime.ts` rebuilds the start request from the record when the in-memory map is empty, re-resolving credentials/context policy live (B-2.8) and workspace rules/approval through the task session service's per-workspace runtime setup. Both `[B-1 repro]` regressions in `test/runtime/cline-sdk/session-restart-regression.test.ts` are unskipped and passing; the baseline characterization test was removed.)

### Implementation record (2026-09-24)

Decisions (confirmed with the operator): reliable mode is
`gitDeliveryPolicy.enabled`; an in-flight attempt that reached the Git phases
finishes under the mode and policy it started with.

- B-4.1 phases (`runtimeCompletionPhaseSchema`): implementation → review →
  verification → committing → integrating → integrated_verification →
  pushing → remote_verification → complete. Status is separate
  (`running | blocked | failed | canceled | complete`); a stopped attempt keeps
  the phase it stopped at and resumes there.
- B-4.2 record (`runtimeCompletionAttemptSchema`, stored at
  `<task worktrees home>/<taskId>/completion/attempt.json`; superseded
  attempts under `completion/attempts/`): attempt id, task, workspace, repo
  identity (path + root commit), worktree, starting commit, candidate tree,
  target ref, mode, policy snapshot + version, session ids (implementation,
  review), evidence by reference (review outcome, verification receipt,
  task/integrated/remote commits, delivery receipt), failure reason, bounded
  transition history, timestamps.
- B-4.3: records use the shared locked-file-system conventions (proper-lockfile
  with stale detection, atomic rename) and carry `schemaVersion: 1`; an
  unknown version is reported, never guessed at or overwritten
  (`migrateCompletionAttempt` is the migration seam). Old Done cards without
  a receipt do not unlock dependents in reliable mode (B-5.9 rule).
- B-4.4: `runtime.startTaskCompletion` / `getTaskCompletion` /
  `cancelTaskCompletion`, shared by the web UI (Commit/PR in reliable mode)
  and the CLI (`task finish`, `task completion`, `task cancel-completion`).
  Ownership is reserved synchronously, so duplicate commands resolve to one
  attempt; a complete attempt is returned as-is. The attempt runs in the
  runtime, so closing the browser does not stop it.
- B-4.5: intent is persisted before each side-effecting phase; the Git phases
  go through the delivery service, whose receipt reuses commits and
  reconciles the remote, so resuming never repeats a side effect. At startup
  `reconcileAfterRestart` settles attempts left running: a delivered receipt
  completes them (crash between the side effect and the record); anything
  else becomes `blocked` at its phase with the reason.
- B-4.6: cancel stops a running review/repair session immediately; Git phases
  stop at the next phase boundary (a git process is never killed mid ref
  update). Locks are released by stale detection if a process dies.
- B-4.7: an attempt past `committing` resumes under its recorded policy even
  after reliable mode is turned off, and the web UI routes such a task to the
  coordinator instead of the legacy prompt. Before any Git side effect, a
  policy change starts a fresh attempt and a disabled mode stops at the
  attempt's phase (nothing was published, so the legacy path may proceed).
- Integrated verification: delivery calls a pre-push hook; the coordinator
  pauses at `integrated_verification` if the integrated tree differs from the
  verified tree (fast-forward and the plumbing merge never differ).
- Verification without review: the coordinator runs the checks on their own
  and stores a tree-bound receipt; the delivery gate accepts it.
- Tests: `test/task-completion/completion-coordinator.test.ts` (phases and
  evidence, duplicate starts, blocked/resume, standalone verification,
  delivery-stage mapping, integrated-tree mismatch, cancel during review and
  during Git phases, restart reconciliation, mode/policy versioning, writer
  still running, unknown schema version).
