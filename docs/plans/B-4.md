...
Status note (2026-09-23): only B-4.8 is implemented. B-4.1–B-4.7 (typed
phases, attempt records, one owner per attempt, intent-before-side-effect
reconciliation, per-phase cancellation, mode/policy versioning) remain open.
B-8's per-task delivery receipt covers part of B-4.2/B-4.5 for the Git phases
only; it is not a substitute for the attempt model.
