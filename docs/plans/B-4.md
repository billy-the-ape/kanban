...
The backend knows which completion phase has actually succeeded and can resume safely after an interruption.

### Implementation tasks

- [ ] B-4.1 Define typed completion phases: implementation, review, verification, committing, integrating, integrated verification where needed, pushing, remote verification, and completed. Represent blocked/failed/cancelled states separately and record a resumable phase.
- [ ] B-4.2 Persist attempt ID, task ID, repository identity, starting SHA, candidate tree, target ref, session IDs, policy version, review/check evidence, task commit, integrated commit, remote receipt, timestamps, and failure reason. Store logs by reference.
- [ ] B-4.3 Use existing durable-state conventions and locking. Add schema versioning and migration. Legacy Done cards without a receipt must not automatically unlock new reliable-mode tasks.
...
- [ ] B-4.5 Persist intent before Git side effects and reconcile actual state after restart. Handle a crash between a successful operation and its completion record.
- [ ] B-4.6 Define cancellation per phase. Do not leave locks permanently, and do not assume that killing a Git process reverses a ref update.

### Acceptance and tests

Crash/restart after every transition, including after a side effect but before its receipt is persisted. Duplicate completion commands must not create duplicate commits, pushes, or successor sessions. Unknown evidence pauses rather than guesses success.

### Settings and deployment

Introduce a versioned opt-in reliable-completion mode during rollout. Preserve the legacy/custom-prompt mode explicitly until migration decisions are made. Back up state before schema changes, and make code rollback consider schema compatibility.

### Handoff

Publish the state-transition contract, migration behavior, reconciliation table, and ownership of every side effect.
