...
Part of the **Kanban reliability and sequential delivery implementation plan**. This file contains the B-3 milestone plus the shared plan context needed to understand and execute it. The full plan, including cross-milestone coordination, lives in [B_IMPLEMENTATION_PLAN.md](./B_IMPLEMENTATION_PLAN.md).

| Field | Value |
| --- | --- |
| Document revision | 4 |
| Prepared | 2026-09-15 (revision 2: 2026-09-16, revision 4: 2026-09-20) |
| Status | B-3 implemented and regression-tested (revision 4); shared B-0 deployment progress recorded below |
...
| Upstream approach | Recovery bug fix |
...
- [x] B-3.1 Normalize actual provider error objects, nested causes, codes, and message strings through existing SDK utilities. Two defects are already known and must both be fixed: the instanceof Error guard drops structured and nested provider errors before any pattern runs, and the pattern list matches any message merely mentioning a context window or context length. Classify on provider error code and shape first, and treat message matching as a last resort.
- [x] B-3.2 Prefer the SDK's supported compaction policy. Preserve task requirements, rules, relevant decisions, phase, outstanding findings, file references, and tool call/result pairing. Retain the original transcript separately for inspection.
- [x] B-3.3 Replace or retire the message-count-halving fallback only after proving the SDK path covers its cases. If a fallback remains, make it token-budget-aware and explicit about omitted history.
- [x] B-3.4 Bound the summarizer request itself. Use persisted handoff/checkpoint data and bounded chunks if the history already exceeds capacity. If essential pinned material cannot fit, pause with an actionable reason rather than silently discarding it.
- [x] B-3.5 Retry only the failed inference request with a smaller valid context and a fixed recovery limit. Do not repeat completed tools, edits, commits, or pushes. If a tool's completion is uncertain, reconcile or pause before retry.
- [x] B-3.6 Preserve provider selection, model, tool policies, rules, cancellation, and task/attempt mapping when restarting. A canceled task must not be revived by recovery.
...
## Verified progress — 2026-09-20 (revision 4)

B-3 is **implemented and regression-tested** in the fork. All seven subtasks
are complete; the milestone's acceptance list is covered by the test files
named below (full suite: 78 files, 839 passed / 4 intentionally skipped;
typecheck clean).

- B-3.1 — `isContextOverflowError` (src/cline-sdk/cline-context-recovery.ts)
  classifies structured provider errors first (provider error `code` —
  `context_length_exceeded`, `max_context_length_exceeded`,
  `prompt_too_long` — and HTTP 413), walks the `cause` chain (depth-bounded
  at 8, cycle-guarded), and unwraps `{ error: ... }` gateway envelopes.
  Message matching is the last resort: 14 narrow patterns, each tying the
  failure to the model's context/input size; a bare mention of "context
  window" (UI panels, file names) no longer matches.
- B-3.2 — the recovery path compacts with
  `compactClineConversationMessages` (src/cline-sdk/cline-compaction-callback.ts),
  the same deterministic, token-budget-aware compactor as the B-2.5 SDK
  `compact` callback and local-mode beforeModel hook: it targets the
  calibrated trigger, protects the first user message (original task
  requirements; truncated only as a last resort), repairs orphaned
  tool_result blocks (providers reject them), and prepends the compaction
  notice to the surviving first message.
- B-3.3 — the message-halving fallback
  (src/cline-sdk/cline-context-overflow-compaction.ts) is retired (file
  deleted); the only remaining compaction is the token-budget-aware
  deterministic compactor above, and it is explicit about omitted history
  via the notice.
- B-3.4 — `evaluateClineRecoveryRequirements` pauses recovery with an
  actionable reason when the original task requirements (first user message)
  alone exceed the calibrated compaction target, instead of silently
  truncating them ("...recovery will not silently truncate the original
  requirements").
- B-3.5 — `retryAfterContextOverflow` (src/cline-sdk/cline-task-session-service.ts)
  is bounded by `contextRecoveryMaxAttempts` (constructor-validated positive
  integer, default 3). Only the failed inference request is retried: each
  attempt re-reads the persisted transcript (which includes the failed
  resend, because the SDK persists user prompts before the inference call)
  and re-compacts it, so the restarted request only gets smaller. Non-overflow
  failures during a recovery attempt surface immediately without consuming
  another attempt. Exhaustion fails with
  "Context overflow recovery failed after N attempt(s): the conversation
  still exceeds the context window after compaction. Reduce the conversation
  or task size, or use a model with a larger context window."
- B-3.6 — a turn the user just canceled (`pendingTurnCancelTaskIds`, recorded
  by `cancelTaskTurn` before the abort surfaces) is never revived by
  recovery. The recovery restart reuses the full saved session config via
  the B-2.8 path (provider, model, credentials, baseUrl, reasoning effort,
  cwd, mode, execution policy, resolved system prompt with rules, calibrated
  compaction), so no capability is lost on restart.
- B-3.7 — `evaluateClineContextRecoveryBudget` measures the pinned restart
  material (system prompt + restart prompt + images) against the effective
  input budget (window − reserve − safety margin, B-2 math) BEFORE any
  bounded attempt is burned; when it cannot fit, recovery pauses with the
  specific reason ("...removing conversation history cannot make this turn
  fit") instead of restarting.

Test evidence (no provider subprocess; deterministic fake SDK host):

- test/runtime/cline-sdk/context-overflow-regression.test.ts (33 tests):
  classification positives/negatives (OpenAI, llama.cpp issue #504,
  structured codes, envelopes, cause chains, HTTP 413, self-referencing
  cause cycle, "context window panel" non-match); compactor unit tests
  (few-huge-messages shrink, no-op on estimator drift and empty input,
  orphan tool_result repair, paired tool_use/tool_result survival,
  shrink-only tokens); pure budget-verdict tests for B-3.4/B-3.7; and
  service-level recovery: both overflow shapes recover with a strictly
  smaller restarted request and no first-turn prompt replay, repeated
  overflow hits the default 3-attempt cap (and a custom cap of 1) with the
  exact failure message, non-overflow errors (send path and mid-recovery)
  surface immediately, a canceled overflow is reported but never revived,
  and B-3.4/B-3.7 pause with their actionable reason strings.
- test/runtime/cline-sdk/session-restart-regression.test.ts: new B-3.6
  regression asserting the recovery restart reuses the full saved session
  config (deep-equal on provider/model/credentials/baseUrl/reasoning
  effort/cwd/mode/enable flags/execution/systemPrompt/calibrated
  compaction, new session id, function-valued hooks excluded).
- test/runtime/cline-sdk/cline-task-session-service.test.ts: the
  "compacts persisted history and retries send" test was updated from the
  retired halving fallback (old notice text, unconditional shrink) to the
  B-3 compactor: explicit compaction config on the start request, oversized
  transcript, deletion of old assistant messages, B-2.5 compaction notice.
- test/utilities/cline-session-service-harness.ts: forwards
  `contextRecoveryMaxAttempts` to the service (the harness gap that
  previously prevented testing a custom cap).

Settings: the bounded attempt count defaults to 3 and is configurable via
`createInMemoryClineTaskSessionService` options; exhaustion lands the task
in the explicit `error` review state with the actionable message above.
Existing transcripts are never overwritten: recovery reads the persisted
transcript, compacts an in-memory copy, and restarts a NEW session while the
old session record (and its persisted messages) remains readable.

## Review follow-up — 2026-09-23

- Recovery after a Kanban process restart: the task service no longer keeps
  its own in-memory copy of the start request. `retryAfterContextOverflow`
  asks the session runtime for `resolveRestartStartRequest(taskId)`, which is
  the in-memory request or, after a restart, the one reconstructed from the
  durable session record (B-4.8) with the live context/compaction policy
  (B-2.8). Previously the budget check and compaction were silently skipped
  after a restart and the same oversized transcript was resent.
- Unknown context window: recovery now fails immediately with an actionable
  reason ("the model's context window is unknown...") instead of burning the
  bounded attempts on identical resends.
- Tests: `context-overflow-regression.test.ts` covers both cases.
- B-3.5 uncertain tool completion: when the persisted transcript holds a
  tool call with no recorded result, recovery pauses with an actionable
  reason ("...have no recorded result, so it is unknown whether they ran")
  instead of compacting the dangling call away and resending
  (`findClineUnresolvedToolCalls` in `cline-context-recovery.ts`).
