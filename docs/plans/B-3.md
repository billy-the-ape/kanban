...
- [ ] B-3.1 Normalize the actual provider error object, nested cause, code, and message string through existing SDK utilities. Avoid treating an unrelated mention of "context" as overflow.
- [ ] B-3.2 Prefer the SDK's supported compaction policy. Preserve task requirements, rules, relevant decisions, phase, open findings, file references, and tool-call/result pairing. Keep the original transcript separately for inspection.
- [ ] B-3.3 Replace or retire the message-halving fallback only after proving the SDK path covers those cases. If the fallback remains, make it token-budget-aware and explicit about omitted history.
- [ ] B-3.4 Bound the summarizer's own request. If history already exceeds capacity, use persisted handoff/checkpoint data and bounded chunks; if essential pinned material does not fit, pause with an actionable reason instead of silently dropping it.
- [ ] B-3.5 Retry only the failed inference request with a smaller valid context and a fixed recovery limit. Do not replay completed tools, edits, commits, or pushes. Reconcile or pause before retry when a tool's completion is uncertain.
- [ ] B-3.6 Preserve provider choice, model, tool policy, rules, cancellation, and task/attempt mapping across restart. Cancellation must not be revived by recovery.

### Acceptance and tests

Cover few messages with huge content, paired tool messages, first-task requirements longer than 300 characters, repeated overflow, summarizer failure, nested provider errors, cancellation, and filesystem-state preservation. Assert that actual request size shrinks and completed tools are not replayed.

### Settings and deployment

Propose a bounded number of context-recovery attempts and an explicit recoverable-error state after exhaustion. Existing transcripts remain available and are not irreversibly overwritten by compaction.

### Handoff

Document the chosen strategy and what it intentionally omits. Report preserved invariants and the next action for a task that is irreducibly too large.
