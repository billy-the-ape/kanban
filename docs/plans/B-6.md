...
- [x] B-6.1 Persist an implementation handoff with task ID, authoritative Markdown paths and revisions/hashes, acceptance criteria, changed paths, design decisions, tests attempted, known limitations, and unresolved questions.
- [x] B-6.2 Start a new review session in the same preserved workspace after the implementation writer has stopped. Carry rules and permissions forward; do not append the entire implementation transcript.
- [x] B-6.3 Review against the task's recorded starting revision, including uncommitted and untracked content. Inspect relevant cross-file context on demand; paginate large diffs and retain a coverage checklist.
- [x] B-6.4 Require a structured result: findings with file/line evidence, blocking status, fixes applied, requirements covered, and unresolved items. A parsing failure is not a pass.
- [x] B-6.5 Permit scoped fixes only. Default maximum repair rounds should be finite, initially two for evaluation. Avoid broad refactors, new requirements, dependency upgrades, and unrelated formatting unless the task requires them.
- [x] B-6.6 Deny Git publication and worktree cleanup through review-phase tool policy. Keep existing project rules and tool approvals effective. Command filtering is not a complete security sandbox; preserve existing OS isolation too.
- [x] B-6.7 Bind the review result to the candidate content/tree. Any subsequent edit invalidates the result or requires explicit scoped re-review. At limit exhaustion, retain the workspace and findings.
...
- `src/cline-sdk/cline-review-session-service.ts` orchestrates the review:
  handoff artifact (B-6.1), fresh session per run (a finished session is
  cleared first so a second review never reuses the old context), scoped
  repair rounds (B-6.5), a parsed `kanban-review-result` block where a parse
  failure is `parse_failed` (B-6.4), and a durable outcome bound to the
  candidate tree hash (B-6.7). One review per task at a time; each turn is
  bounded by a timeout (default 30 minutes) after which it is stopped and
  reported as failed.
- B-6.2: a review refuses to start while the task's own Cline or terminal
  agent session is still running.
- B-6.6: `review-tool-policy.ts` only adds denials (push/commit/merge/rebase/
  update-ref/tag, reset --hard, forced clean, stash, checkout/switch/restore of
  worktree files, worktree remove/prune, destructive branch edits,
  `gh pr create/merge`, `rm -rf .`/`.git`, and edits outside the reviewed
  change set). Everything else goes to the workspace's existing approval
  handler, so project approvals stay effective.
- B-6.7 enforcement lives in B-8: when review is `required`, delivery pauses
  unless the stored verdict is `ready` and bound to the exact tree being
  committed.
- Entry points: `runtime.startTaskReview`/`getTaskReviewInfo`, the
  `kanban task review --task-id <id>` CLI, and a Review button with a
  verdict badge (ready / blocked / failed / stale once the worktree changes)
  in the task detail diff toolbar for Review-column tasks.
