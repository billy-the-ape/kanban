...
Part of the **Kanban reliability and sequential delivery implementation plan**. This file contains the B-6 milestone plus the shared plan context needed to understand and execute it. The full plan, including cross-milestone coordination, lives in [B_IMPLEMENTATION_PLAN.md](./B_IMPLEMENTATION_PLAN.md).
...
| Prerequisites | B-3, B-4, B-5 |
| Upstream approach | Feature discussion if required |
...
- [ ] B-6.1 Persist an implementation handoff containing task ID, authoritative Markdown path and revision/hash, acceptance criteria, changed paths, design decisions, attempted tests, known limitations, and open questions.
- [ ] B-6.2 Start a new review session in the same preserved workspace after the implementation writer stops. Carry rules and permissions, but do not append the full implementation transcript.
- [ ] B-6.3 Review against the task's recorded starting revision, including uncommitted and untracked content. Inspect relevant cross-file context on demand. Page large diffs and preserve a coverage checklist.
...
- [ ] B-6.5 Permit only scoped fixes. Use a finite default maximum repair-round setting, initially two for evaluation. Avoid broad refactors, new requirements, dependency upgrades, and unrelated formatting unless the task requires them.
- [ ] B-6.6 Deny Git publication and worktree cleanup in the review-phase tool policy. Existing project rules and tool approvals remain effective. Command filtering is not a complete security sandbox; preserve existing OS isolation.
- [ ] B-6.7 Bind the review result to candidate content/tree. Later edits invalidate it or require an explicit scoped re-review. Preserve workspace and findings when the limit is exhausted.

### Acceptance and tests

A seeded bug is found in review, repaired, and handed to verification without invoking Git delivery. Test fresh-context identity, large multi-file diffs, missing requirements, malformed review output, policy violations, and repair-limit exhaustion.

### Settings and deployment

Proposed review policy: required/off, review instructions, model override only when explicitly configured, and maximum repair rounds. The target workflow uses required review and the existing local model. It must never silently switch to a cloud model.
