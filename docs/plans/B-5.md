...
- [ ] B-5.1 Replace clean-worktree success inference with a completion receipt in reliable mode. Audit use-review-auto-actions.ts and every CLI/API Done/delete path, not just the visible button.
- [ ] B-5.2 Establish a durable task branch or namespaced recovery ref before relying on an ephemeral detached HEAD. Preserve task ID, workspace path, starting SHA, latest known commit, and preservation status.
...
- [ ] B-5.4 Reconcile unexpected external commits or branch changes. A clean workspace containing an unintegrated manual commit is recoverable work, not evidence to delete.
- [ ] B-5.5 Separate task completion from workspace garbage collection. Default failed/blocked workspaces to preserved. Cleanup requires durable recovery, no active writer, and delivery evidence appropriate to the mode.
- [ ] B-5.6 Restore from the preserved task revision rather than recreating a worktree from an older base. Expose supported locate/recover actions.
- [ ] B-5.7 Define explicit discard behavior and retention limits. Retention expiry must not silently discard unpreserved work; provide a visible blocked-cleanup reason.

### Acceptance and tests

Reproduce a manual commit while auto-completion is armed, hidden/untracked and binary files, failed preservation write, stale metadata, and service restart during cleanup. Restore produces the expected task content, and a failed task does not automatically start dependents.

### Settings and deployment

Document workspace retention and recovery storage. Default to preserving failed attempts, and verify disk-full behavior. This milestone can ship independently as a data-preservation bug fix.

### Handoff

Include a tested recovery procedure and evidence that the cleanup predicate is enforced across UI, CLI, bulk cleanup, and shutdown paths.
