...
- [x] B-5.2 Establish a durable task branch or namespaced recovery ref before relying on ephemeral detached HEAD state. Retain task ID, workspace path, starting SHA, latest known commit, and preservation status.
- [x] B-5.3 Preserve uncommitted tracked changes, untracked files, and relevant binary content separately from Git refs. A patch alone may be insufficient. Automatic preservation failure must stop cleanup.
- [x] B-5.4 Reconcile unexpected external commits or branch changes. A clean workspace containing an unintegrated manual commit is recoverable work, not evidence to delete it.
- [x] B-5.5 Separate task completion from workspace garbage collection. Default failed/blocked workspaces to retained. Cleanup requires durable recovery, no active writer, and delivery evidence appropriate to the mode.
- [x] B-5.6 Make restore use the preserved task revision rather than recreating a worktree from an older base. Expose a supported locate/recover action.
- [x] B-5.7 Define explicit disposal behavior and retention limits. Retention expiry must not silently discard unpreserved work; provide a visible blocked-cleanup reason.
...
- [x] B-5.9 Make the trash/Done transition atomic and correctly ordered. trashTaskById currently mutates board state, stops the session, starts every ready dependent, and only then deletes the workspace, so a failure while starting a dependent leaves the card completed, the worktree alive, and dependents half-started with nothing recorded. Persist the intended transition first, dispatch dependents only after delivery evidence exists per B-9, and make workspace deletion a separate reconcilable step whose failure is visible.
...
- B-5.2/B-5.3/B-5.4/B-5.6 (`task-preservation.ts`, `task-patch.ts`): a
  recovery ref `refs/kanban/tasks/<id>` is kept current while the worktree
  lives; a patch plus a full tar archive are written before removal; restore
  uses the preserved revision; `task locate`/`task recover`. Acceptance tests
  in `test/integration/task-preservation.integration.test.ts`: modified,
  deleted, hidden, untracked, and binary content restores byte-for-byte; a
  clean worktree holding an unintegrated manual commit stays recoverable and
  restores at that commit; a failed archive write blocks cleanup with a
  durable reason. B-8 uses its own ref namespace (`refs/kanban/delivery/<id>`):
  git cannot hold both `refs/kanban/tasks/<id>` and `refs/kanban/tasks/<id>/...`.
- B-5.5: cleanup is preservation-gated, refuses while the task's agent or
  terminal session is still running (`workspace.deleteWorktree`), and
  completion never removes a worktree by itself; failed or blocked tasks are
  never cleaned up automatically.
- B-5.9: the board move is persisted before any side effect; dependents start
  only when the runtime reports delivery evidence
  (`getTaskDeliveryInfo().dependentsUnlock`, used by the web UI and
  `task complete`); discarding never starts them. Worktree deletion is a
  separate reconcilable step: a blocked cleanup is recorded on the
  preservation record (`cleanupBlockedReason`), shown on the Trash card, and
  retried by workspace maintenance.
- B-5.7 retention (`task-workspace-maintenance.ts`, run at runtime startup,
  after completion, and by `kanban task cleanup`):
  - a Done task's worktree is removed once its delivery receipt is complete
    (preservation-gated; the receipt plus the archive are the recovery path);
  - preservation assets of delivered or discarded work are pruned 30 days
    after delivery/discard, and oldest-first while this workspace's assets
    exceed 5 GiB;
  - undelivered work (still on the board outside Trash, or Done without a
    receipt), including failed and blocked tasks, is never pruned
    automatically; past the retention period or when the cap cannot be met
    it is reported as flagged instead.
