...
| Area | Location |
| --- | --- |
| Readiness + queue pass (B-9.1/B-9.2/B-9.3) | `src/task-dispatch/task-dispatch-service.ts` — `resolveReadyTasks`, `dispatchReadyTasks` (per-workspace lock, backlog order, `workerLimit` slots held by running / awaiting-review model sessions; home-agent sessions never hold a slot) |
...
| Durable records (B-9.6) | `src/task-dispatch/dispatch-records.ts` — atomic JSON record per task, written **before** the board move and session start; statuses `dispatching / dispatched / failed / blocked / exhausted`; retry cap 3 (`TASK_DISPATCH_RETRY_CAP`) |
| Restart reconciliation (B-9.6) | `reconcileTaskDispatch` — in-progress tasks with a record, no live session, and no delivery receipt are relaunched from the recorded base, reusing the recorded prompt; runs post-startup for every managed workspace (`src/server/runtime-server.ts`) |
...
| Board edge retention | `src/core/task-board-mutations.ts` (`updateTaskDependencies`, `allowDoneEndpoints: true`) — edges to done tasks survive normalization so the backend can resolve prerequisites after completion (trash edges still drop) |
| Config plumbing | `src/core/api-contract.ts` (policy + record + response schemas), `src/config/runtime-config.ts` (defaults / normalize / validate / merge / save), `src/terminal/agent-registry.ts` (response pass-through) |
...
Evidence: `test/task-dispatch/task-dispatch-service.test.ts` (32 unit tests: linear
chain, diamond, cycle guard, missing/paused/failed/no-op receipts, cross-workspace
receipt rejection, slot accounting, prompt content, dispatch/failure/retry-cap/
stale-base orchestration, reconciliation cases, status view) and
`test/integration/task-dispatch.integration.test.ts` (4 tests: real git repo + real
board/records/receipts — dispatch at the verified base, restart reconciliation
reusing the recorded prompt, failed-push receipt keeping the successor blocked,
re-delivery dispatching the successor with full delivered ancestry in the new
worktree). Both suites run with `HOME` redirected to a temp dir.
