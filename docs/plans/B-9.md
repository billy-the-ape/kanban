...
An unattended chain reliably builds on the prior verified commit, using a fresh context window for each task.

### Implementation tasks

- [ ] B-9.1 Move reliable-mode readiness decisions into the backend. Consume completion receipts, not card deletion or a browser-only timer.
- [ ] B-9.2 Verify dependency direction, missing tasks, cycles, failed prerequisites, and cancellation. A manually moved Done card is not a substitute for required delivery evidence.
- [ ] B-9.3 Reserve one worker across implementation, review, repair, and model-based compaction by default. Git-only phases do not need a model slot but must still respect repository locks.
- [ ] B-9.4 Resolve the next task's base only when prerequisites are satisfied. Verify that the selected revision contains their delivered results. Do not reuse a stale pre-created worktree.
- [ ] B-9.5 Create a new session with the current task document, selected global requirements, rules, concise prerequisite handoff, and exact base SHA. Do not preload the entire feature plan or prior transcripts indiscriminately.
...
- [ ] B-9.7 Ensure a failed push, failed review, or interrupted verification blocks successor execution even if the predecessor's workspace is clean.

### Acceptance and tests

A three-task fixture requires task 2 to use task 1's API and task 3 to extend task 2. Each starts from the correct delivered revision and a different session. Repeat with browser closed, duplicate clients, service restart, and a push failure mid-chain. Model concurrency must remain at one.

### Settings and deployment

Enable the reliable queue per project. The worker limit defaults to one for this workflow. Dependency metadata is durable and independent of visual column placement.

### Handoff

Record the three-task run trace, base/commit relationships, session IDs, and restart proof.
